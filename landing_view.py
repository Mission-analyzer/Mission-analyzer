...
def draw_landing_approach(canvas: tk.Canvas, analyzer: MissionAnalyzer, n_legs: int = 3):
    ...
    for i in range(6):
        ...
        y = Y(val)
        canvas.create_line(margin_l, y, width - margin_r, y, fill="#e8e8e8")
        canvas.create_text(margin_l - 6, y, text=f"{val:.0f}", anchor="e", font=("Arial", 8))

    # рельеф (заливка), если есть SRTM
    if has_terrain:
        floor_y = Y(y_min)
        poly = [
            (X(d), Y(t)) for d, t in zip(elevation["dist"], elevation["terrain_alt"]) if t is not None
        ]
        if poly:
            border = [(X(elevation["dist"][0]), floor_y)] + poly + [(X(elevation["dist"][-1]), floor_y)]
            flat = [c for p in border for c in p]
    ...
    for i in range(len(pts_xy) - 1):
        canvas.create_line(*pts_xy[i], *pts_xy[i + 1], fill="#1f77b4", width=2.5)
    ...
    for i, leg in enumerate(legs):
        ...
        canvas.create_text(xm, row_y, text=label, font=("Arial", 8), fill="#333333")
    ...
    for marker in speed_markers:
        ...
        canvas.create_line(x, margin_t, x, height - margin_b, fill="#2ca02c", dash=(3, 2))
        speed_label = i18n.t(
        ...
        canvas.create_text(x, speed_row_y, text=speed_label, font=("Arial", 8, "bold"), fill="#2ca02c")
    ...
    for p in points:
        ...
        color = "#d62728" if p["is_land"] else "#1f77b4"
        canvas.create_oval(x - 5, y - 5, x + 5, y + 5, fill=color, outline="white")
        canvas.create_text(x, y + 13, text=str(p["seq"]), font=("Arial", 8, "bold"))
        canvas.create_text(x, y + 25, text=p["command_name"], font=("Arial", 7), fill="#666666")

    canvas.create_text(width / 2, title_y, text=i18n.t("title_landing_approach"), font=("Arial", 11, "bold"))
