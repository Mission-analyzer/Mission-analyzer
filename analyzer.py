...
class MissionAnalyzer:
    ...
    def flight_path_angle_profile(self) -> list[dict]:
        ...
        Точка LAND сюда НЕ входит: заход на посадку — крутий, різкий
        відрізок (десятки градусів), і на його фоні звичайний допуск
        у ±2° стає непомітним -- шкала графіка розтягується під LAND,
        а корисна частина маршруту "з'їдається". Для самого заходу на
        посадку є окрема перевірка й окремий графік -- глісада
        (landing_glide_profile/сторінка "Посадка"), тут вона не потрібна.
        """
        pts = [wp for wp in self.nav_wps if wp.command not in LAND_COMMANDS]
        if len(pts) < 2:
